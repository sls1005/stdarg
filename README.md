...
+ For each `VAList` initialized with `init()`, `va_end` will be automatically invoked. For a `VAList` initialized with another method, `va_end` must be called once in the *same proc* as it initialized. However, `va_end` don't need to be invoked if the list is never initialized.
